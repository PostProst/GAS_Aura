...
void UMVVM_LoadScreen::NewSlotButtonPressed(int32 Slot)
{
	...
	LoadSlotsMap[Slot]->SetMapName(AuraGameMode->DefaultMapName);
	LoadSlotsMap[Slot]->PlayerStartTag = AuraGameMode->DefaultPlayerStartTag;
	AuraGameMode->SaveSlotData(LoadSlotsMap[Slot], Slot);
	LoadSlotsMap[Slot]->InitializeSlot();

	// make slot selected after it has been created
	SelectSlotButtonPressed(Slot);

	UAuraGameInstance* AuraGameInstance = Cast<UAuraGameInstance>(AuraGameMode->GetGameInstance());
	AuraGameInstance->LoadSlotName = LoadSlotsMap[Slot]->GetLoadSlotName();
	AuraGameInstance->LoadSlotIndex = LoadSlotsMap[Slot]->LoadSlotIndex;
...
void UMVVM_LoadScreen::PlayButtonPressed()
{
	if (IsValid(SelectedLoadSlot))
	{
		...
		AuraGameInstance->LoadSlotName = SelectedLoadSlot->GetLoadSlotName();
		AuraGameInstance->LoadSlotIndex = SelectedLoadSlot->LoadSlotIndex;
